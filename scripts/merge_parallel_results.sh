#!/bin/bash
output_file="parallel_results.txt"  # Name of the output file
...
for ((N = 1; N <= 32; N *= 2)); do # Loop for N with powers of 2
    for ((DIM = 1; DIM <= 1024; DIM *= 2)); do # Loop for DIM with powers of 2
        for file in run_parallel_${N}_${DIM}.sh.o*; do
            [ -e "$file" ] && cat "$file" >> "$output_file"
        done
    done
done

echo "All 'run_parallel' files combined into $output_file"